...
def test_merge_two_sorted_lists():
    ...
    for i in range(500):
        ...
        assert m.merge_two_sorted_iterables(sorted(main_lst[0]), sorted(main_lst[1])) == sorted(main_lst[0] + main_lst[1])
...
def test_merge_k_sorted_lists():
    ...
    for i in range(500):
        ...
        assert m.merge_k_sorted_iterables([sorted(lst) for lst in main_lst]) == sorted(list(chain.from_iterable(main_lst)))

...
from .currency_proc_config import CURRENCY_NAMES, EUR_RE, POUND_RE, CZ_CROWN_RE
...
# def store_currencies_info(currencies: CURRENCIES) -> str:
#     filename = CI_INFO_PATH
...
def find_currency_in_str(string: str) -> str:
    if EUR_RE.search(string) is not None:
        return "EUR"

    if POUND_RE.search(string) is not None:
        return "GBP"

    if CZ_CROWN_RE.search(string) is not None:
        return "CZK"
